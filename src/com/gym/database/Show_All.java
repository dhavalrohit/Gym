...
public class Show_All extends JFrame{
	...
	public Show_All() {
		...
		Member_Table.setModel(new DefaultTableModel(
			...
			new String[] {
				"ID", "Membership No", "Full Name", "Gender", "Registered Date", "Total fee", "Contact No", "Area", "Address", "Email"
			}
		));
		...
		btnSearch.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				String sql="Select id, membership_no, full_name, gender, reg_date, total_fee, contact_no, area,address, email from members where membership_no='"+txtSearch.getText()+"'";
		        try{
		        	con=Connect.connectDb();
		            pst=con.prepareStatement(sql);
		            rs=pst.executeQuery();
		...
		btnLatest.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				try {
					con=Connect.connectDb();
					String sql="Select id as'ID',membership_no as 'Membership No',full_name as 'Full Name',gender as 'Gender',reg_date as 'Registered Date',total_fee as 'Total Fee',contact_no as 'Contact No',area as 'Area',address as 'Address',email as 'Email' from members order by id desc";
			            con=Connect.connectDb();
			            pst=con.prepareStatement(sql);
			            rs=pst.executeQuery();
